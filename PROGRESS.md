...
| 태스크 | 무엇을 | 왜 할 만한가 |
|---|---|---|
| T-3.6 촬영 시각 기반 경기 추천 (REQ-F-204) | 사진 EXIF 의 촬영 시각을 읽어 그날 경기를 후보로 제시 | 이미 EXIF 를 지우고 있으니 **지우기 전에 읽으면** 된다. 발표에서 "사진만 올리면 어느 경기인지 알아낸다" 가 잘 먹힌다 |
| T-6.4 실시간 대화 수신 (REQ-F-512) | WebSocket. 지금은 새로고침으로 읽는다 | 시간이 좀 걸린다 |

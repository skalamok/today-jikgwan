...
로고가 이미 티켓이고 직관 카드도 티켓 모양이다. 다른 방향으로 가면 로고와 화면이 따로 논다.
서비스 성격도 "쌓인다" 이고 화면 대부분이 기록 · 표 · 집계다.

어두운 화면(야구장의 밤)은 예쁘지만 읽어야 하는 화면이 많고 낮에도 쓴다.
스포츠 앱의 선명함은 실시간 스코어 중심에 맞는데 실시간은 범위에서 뺐다.

결이 되는 것 — 종이 질감의 밝은 바탕 · 네이비 잉크 · 절취선 · 도장처럼 찍히는 결과 배지 ·
표는 장부처럼 정렬. 로고의 네이비 `#16355c` 가 기준색이다.

### three.js 는 쓰지 않는다

쓸 자리는 구장 좌석 구역을 3D 로 보여주는 것 하나다. "어느 자리가 좋은가" 가 핵심
기능이라 값어치도 있다. 그런데 **구장이 아홉 개다.** 하나만 만들어 돌려쓰면 실제 배치가
아니게 되고, 발표에서 "이건 실제 모양이 아닙니다" 를 설명하는 순간 값어치가 사라진다.
아홉 개를 다 만들 시간은 없다.

같은 목적이면 **2D 좌석 배치도**가 정직하고 빠르다. 구역 만족도를 색으로 칠하면
3D 보다 오히려 읽기 쉽다. 이쪽으로 간다.

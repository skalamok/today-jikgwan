...
에픽 태스크 49개 — 완료 41 · 진행 5 · 예정 3. **MVP 태스크는 모두 완료다.**
...
| 태스크 | 무엇을 | 왜 할 만한가 |
|---|---|---|
| T-6.4 실시간 대화 수신 (REQ-F-512) | WebSocket 으로 올리기. **지금도 동작한다** — 5초 증분 폴링으로 새 메시지를 받고 화면을 떠나면 멈춘다. 요구사항 비고가 "미적용 시 주기적 조회로 대체 가능" 이라 이대로도 어긋나지 않는다 | 급하지 않다 |
| T-4.5 날씨별 전적 마무리 | 함께 간 사람별은 됐다. 날씨별은 기상청 키가 필요하다 | 키가 오면 바로 |

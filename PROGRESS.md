...
## 다음 세션에서 할 것 — 프런트 다시 입히기

**다시 짜는 것이 아니다.** 화면 20개는 잘 돌아간다. 문제는 보이는 것뿐이다.

사용자가 꼽은 불만 넷 — 밋밋하다 · 모바일이 어설프다 · 화면마다 스타일이 다르다 ·
데모 흐름이 매끄럽지 않다 — 은 원인이 하나다. **기준이 되는 디자인이 없어서 화면마다
즉흥적으로 스타일을 붙였다.** 기준을 만들고 입히면 넷이 같이 풀린다.

목표는 "실제로 배포한 서비스처럼 보이는 것" 이다.

### 순서

1. **토큰을 정한다** (`src/style.css`)
   색 · 간격 · 모서리 · 그림자 · 글자 크기를 CSS 변수로. 지금은 화면마다 `#16355c`,
   `#d8dde3` 같은 값을 직접 적고 있다. 로고의 네이비를 기준색으로 삼는다.

2. **공통 컴포넌트를 뺀다** (`src/components/`)
   `Button` · `Card` · `Field` · `Chip` · `Stars` · `EmptyState`.
   지금은 같은 버튼이 화면마다 조금씩 다르게 적혀 있다.
...
- `src/lib/ticketCard.js` 의 카드 도안. 로고와 같은 티켓 실루엣이라 따로 논다면 그때 본다
- 백엔드. 이번 작업은 프런트만이다
